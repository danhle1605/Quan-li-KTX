...
class Payment extends Model {
    ...
    public function getAll($keyword = '', $status = '', $page = 1, $limit = 6, $roomId = null) {
        $offset = ($page - 1) * $limit;
        $params = [];
        $whereClause = [];
    ...
    public function belongsToRoom($id, $roomId) {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM invoices WHERE id = :id AND room_id = :room_id");
        $stmt->execute([':id' => $id, ':room_id' => $roomId]);
        return (bool)$stmt->fetchColumn();
    }

    public function existsForRoomMonth($roomId, $billingMonth) {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM invoices WHERE room_id = :room_id AND billing_month = :billing_month");
        $stmt->execute([':room_id' => $roomId, ':billing_month' => $billingMonth]);
        return (bool)$stmt->fetchColumn();
    }
    ...
    public function create($data) {
        try {
            [$month, $year] = explode('/', $data['billing_month']);
            $invoiceCode = 'INV-' . $year . $month . '-' . ($data['room_number'] ?? rand(100,999));
            $total = (float)$data['room_fee'] + (float)$data['electricity_fee'] + (float)$data['water_fee'];

                $status = ($data['status'] ?? 'Unpaid') === 'Paid' ? 'Paid' : 'Unpaid';
                $paidAt = $status === 'Paid' ? date('Y-m-d H:i:s') : null;
                $sql = "INSERT INTO invoices (invoice_code, room_id, billing_month, room_fee, electricity_fee, water_fee, total_amount, status, paid_at) 
                    VALUES (:invoice_code, :room_id, :billing_month, :room_fee, :electricity_fee, :water_fee, :total_amount, :status, :paid_at)";
            $stmt = $this->db->prepare($sql);
            $result = $stmt->execute([
